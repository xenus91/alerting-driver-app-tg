...
// Глобальная переменная для отслеживания последней проверки
let lastAutoCheck = 0
const AUTO_CHECK_INTERVAL = 2 * 60 * 1000 // 2 минуты между автоматическими проверками
  ...
  try {
    ...
    if (now - lastAutoCheck < AUTO_CHECK_INTERVAL) {
      return NextResponse.json({
        success: true,
        message: "Too soon for auto check",
        nextCheck: new Date(lastAutoCheck + AUTO_CHECK_INTERVAL).toISOString(),
      })
    }
    ...
    console.log(`=== AUTO CHECK COMPLETED: ${notificationsSent} notifications sent ===`)

    return NextResponse.json({
      success: true,
      checked: subscriptions.length,
      sent: notificationsSent,
      nextCheck: new Date(now + AUTO_CHECK_INTERVAL).toISOString(),
    })
  } catch (error) {
    console.error("Error in auto check subscriptions:", error)
    return NextResponse.json({ success: false, error: "Failed to auto check subscriptions" }, { status: 500 })
  }
}
