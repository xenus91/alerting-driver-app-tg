...
export async function POST() {
  try {
    ...
    // Используем Pages API endpoint
    const webhookUrl = `${appUrl}/api/tg-webhook`

    console.log("Setting PAGES API Telegram webhook to:", webhookUrl)
    ...
    try {
      ...
      // Тестируем доступность Pages API endpoint
      ...
      try {
        ...
        console.log("Pages API endpoint test:", testResponse.status, testResponse.ok)
      } catch (testError) {
        console.log("Pages API endpoint test failed:", testError)
      }
      ...
      console.log("Pages API webhook setup response:", data)
      ...
      if (data.ok) {
        ...
        return NextResponse.json({
          success: true,
          message: "Pages API Telegram webhook успешно настроен!",
          url: webhookUrl,
        ...
        return NextResponse.json(
          {
            success: false,
            error: data.description || "Ошибка при настройке Pages API webhook",
    ...
    console.error("Setup pages webhook error:", error)
    return NextResponse.json(
      {
        success: false,
