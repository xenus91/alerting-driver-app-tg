...
export async function POST() {
  try {
    ...
    console.log("Setting PUBLIC Telegram webhook to:", webhookUrl)
    ...
    try {
      ...
      // Тестируем доступность публичного endpoint
      ...
      try {
        ...
        console.log("Public endpoint test:", testResponse.status, testResponse.ok)
      } catch (testError) {
        console.log("Public endpoint test failed:", testError)
      }
      ...
      const response = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/setWebhook`, {
        ...
        body: JSON.stringify({
          url: webhookUrl,
          drop_pending_updates: true,
          // Убедимся, что callback_query здесь есть
      ...
      console.log("Public webhook setup response:", data)
      ...
      if (data.ok) {
        ...
        return NextResponse.json({
          success: true,
          message: "Публичный Telegram webhook успешно настроен!",
          url: webhookUrl,
        ...
        return NextResponse.json(
          {
            success: false,
            error: data.description || "Ошибка при настройке публичного webhook",
    ...
    console.error("Setup public webhook error:", error)
    return NextResponse.json(
      {
        success: false,
