...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    // Собираем данные о рейсах для отправки
    const trips = []
    ...
    // Получаем текст сообщения из базы данных (из первой записи, так как он одинаков для всех)
    const messageText = messagesResult[0].message

    // Отправляем сообщение через Telegram, используя сохраненный текст
    const telegramResult = await sendMultipleTripMessageWithButtons(
      ...
      messageText // Передаем сохраненный текст сообщения
    )
