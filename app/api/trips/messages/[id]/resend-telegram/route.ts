...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const phone = message.phone // Сохраняем номер для фильтрации
    ...
    const pointsResult = await sql`
      ...
      WHERE tp.trip_id = ${message.trip_id} AND tp.trip_identifier = ${message.trip_identifier} AND tp.driver_phone = ${phone}
