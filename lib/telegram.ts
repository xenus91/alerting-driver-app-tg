...
export async function sendMultipleTripMessageWithButtons(
  ...
  trips: Array<{
    ...
    // Новое поле для всех точек в правильном порядке
    points?: Array<{
      point_id: string
      point_name: string
      door_open_1?: string
  ...
  try {
    ...
    sortedTrips.forEach((trip, tripIndex) => {
      ...
      // НОВАЯ ЛОГИКА: Используем points если доступно, иначе fallback на старую логику
      if (trip.points && trip.points.length > 0) {
        console.log(`Using new points logic for trip ${trip.trip_identifier}`)
        ...
        sortedPoints.forEach((point, index) => {
          const typeIcon = point.point_type === "P" ? "📦" : "📤"
          const typeName = point.point_type === "P" ? "Погрузка" : "Разгрузка"

          message += `${index + 1}) ${typeIcon} <b>${point.point_id} ${point.point_name}</b> (${typeName})\n`
        ...
        // Строим маршрут для всех точек
        const routeUrl = buildRouteUrl(sortedPoints)
        if (routeUrl) {
          message += `🗺️ <a href="${routeUrl}">Построить маршрут</a>\n\n`
          console.log(`Added route URL for trip ${trip.trip_identifier}`)
        }
      } else {
        // СТАРАЯ ЛОГИКА: Fallback для совместимости
        console.log(`Using legacy points logic for trip ${trip.trip_identifier}`)

        // Погрузка с адресом и ссылкой
        if (trip.loading_points && trip.loading_points.length > 0) {
        ...
        if (trip.unloading_points && trip.unloading_points.length > 0) {
        ...
        // Строим маршрут для этого рейса (старая логика)
